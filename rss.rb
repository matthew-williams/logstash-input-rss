...
class LogStash::Inputs::Rss < LogStash::Inputs::Base
  ...
  def handle_rss_response(queue, item)
    @codec.decode(item.description) do |event|
      event.set("Feed",  @url)
      ##
      # Published is only a recommended field, not a mandatory
      # one, see https://validator.w3.org/feed/docs/rss2.html for details.
      ##
      event.set("published", item.pubDate) if !item.published.nil?
      event.set("title", item.title)
      event.set("link", item.link)
      ##
      # Author is only a recommended field, not a mandatory
      # one, see https://validator.w3.org/feed/docs/rss2.html for details.
      ##
